...
def test_baseline_train(
    baseline_name: str, baselines_dir: Path, data_path: Path, temp_dir: Path
) -> tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode != 0:
            return (
                False,
                f"Train command failed with code {result.returncode}\nStderr: {result.stderr}",
            )
...
def test_baseline_predict(
    ...
    try:
        ...
        if result.returncode != 0:
            return (
                False,
                f"Predict command failed with code {result.returncode}\nStderr: {result.stderr}",
            )
        ...
        if len(df_pred) != len(df_input):
            return (
                False,
                f"Row count mismatch: {len(df_pred)} predictions vs {len(df_input)} input samples",
            )

        return (
            True,
            f"Generated predictions for {len(df_pred)} samples with {len(property_cols)} properties",
        )
...
def test_baseline(
    ...
    # Test 1: Train
    if not skip_train:
        success, message = test_baseline_train(
            baseline_name, baselines_dir, train_data_path, temp_dir
        )
...
def main():
    ...
    parser.add_argument("--baseline", type=str, help="Test only this specific baseline")
    parser.add_argument(
        "--skip-train",
        action="store_true",
