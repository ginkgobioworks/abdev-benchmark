...
class ESM2TapXGBModel(BaseModel):
    ...
    1. Reducing ESM2 embeddings from 640D to 50D using PCA (retains ~94% variance)
    2. Combining with 5 TAP biophysical features
    3. Adding antibody subtype features (hc_subtype, lc_subtype)
    4. Using XGBoost with strong anti-overfitting hyperparameters

    Key anti-overfitting strategies (Version 3 - Balanced approach):
    - PCA dimensionality reduction (640 → 50)
    - Moderate boosting rounds (50, finding sweet spot between 30 and 100)
    ...
    XGBoost vs Random Forest: XGBoost uses gradient boosting (sequential trees)
    vs RF's bagging (parallel trees), often achieving better performance with
    fewer trees and more built-in regularization options.
    """
